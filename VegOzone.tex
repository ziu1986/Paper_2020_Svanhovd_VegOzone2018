...
The estimated biomass reduction in \unit{\%} for the bespoke temperature response function of type boreal in accordance to \citet{ICP:MappingManual2017, ESPR:Hayes2021} is shown in Table~\ref{tab:biomass_reduction}. The estimated uncertainty range corresponds to the deviation given by the $\pm 20\,\unit{\%}$ variations in light threshold and SWP=on. For downy birch and perennial grassland, substantial reductions in total biomass are predicted for any of the two years. Norway spruce is least affected. The systematic uncertainties for downy birch are one order of magnitude larger than for Norway spruce and perennial grassland. For all species or PFTs the interannual variability is larger than the systematic uncertainties estimated. In comparison with average results for the default MM parameterization (with bespoke GS), we find a non negligible (above systematic uncertainties) low-bias for both downy birch and perennial grassland. For Norway spruce, the default MM parameterization average lies within the given systematic uncertainty range. 

\begin{table}[t]
  \caption{Estimated total biomass reduction in \unit{\%} for bespoke temperature response function of type boreal with bespoke GS and SWP=off. Uncertainty ranges reported here correspond to deviations by the $\pm 20\,\unit{\%}$ variation in light threshold and SWP=on, respectively. For comparison results for MM parameterization are shown with standard deviations over both years and SWP treatments.}
...
The exact order of magnitude of these effects varies for individual species or PFT and between years, but the thus predicted ozone uptake is in any case larger than for MM default parameterizations. This indicates a considerable underestimation of ozone risk for subarctic species.
 The impact of the timing of the GS can be at the same order of magnitude as the acclimation of response function but points into the opposite direction. Especially, the timing of $A_\mathrm{start}$ with respect to the ozone spring peak induces large uncertainties in anticipation of an acclimation to colder climates. 
