...
The described methods for risk assessment rely on accurate representation of plant physiological responses to key environmental variables and are most often parameterized using data for less extreme climates, i.e., temperate, continental, or Boreal climates. Parameterization of these models for more extreme bio-geographical regions such as subarctic climates has not been performed and relies on assuming that less extreme Boreal parameterization will be applicable. It is important to understand the implications of using these in the subarctic. In our study, we explore how risk might be over- or underestimated using Boreal parameterization and explore an adjustment of temperature and light response functions based on climate data. This allows an understanding of which are the key aspects of plant physiology that might determine the potential risk and also how the threat from ozone may change in the future. Our results may hint a solution to solve long-standing issues of earth system models (ESM) which often lack PFTs specifically representing subarctic conditions in their land surface models and display over- or underproductivity in the Arctic \citep{GMD:Poulter2015,JAMES:Lawrence2019}.\\ 

Data and methods including the DO3SE model and the localization of response function are described in Section~\ref{sec:dam}. The model experiments and derived parameterizations are summarized in Section 3. In Section 4, we present our modelling results. We discuss our results and give an outlook on how these could improve future assessments of pollution risk.
...
\end{equation}
%
Stomatal resistance is the inverse of stomatal conductance $g_\mathrm{sto}$ in Eq.~(\ref{eq:stomatal}).

\subsubsection{Input data and gap filling methodology}
\label{subsec:ozone_reco}
The $\mathrm{DO_3SE}$ model requires hourly, continuous ozone and meteorological observations of $2\,\unit{m}$ temperature $T_\mathrm{2m}$, global irradiation $Q_0$, precipitation, $2\,\unit{m}$ wind $u_\text{2m}$, and vapor pressure deficit VPD. VPD has been calculated from observed $T_\text{2m}$ and relative humidity
\begin{equation}
...
\end{equation}
with derived ozone climatology \chem{\left<[O_3]\right>} and anomalies (deviation from the norm) \chem{\Delta[O_3]}. We derived a generalized climatology for northern Fennoscandia from several multi-year records and for our site and determined the station with the highest correlation coefficient to our site. For this site, we computed the anomalies compared to the climatology for the dates of missing ozone data at our site. These anomalies were than added to the climatology at out site and corrected for an increase in tropospheric background ozone since the early 1990s. For more details refer to as \citet{ACPD:Falk2021}.
%%%
...
\section{Model parameter localization}
\label{sec:do3se_parameters}
