...
\subsection{Input data and gapfilling methology}
...
\end{equation}
with satuation vapor pressure $P_s(T)$ in \unit{hPa} derived from Arden Buck equation~\citep{JAP:Buck1981, Buck2012}.
  
The following gap filling methodology was devised for the meterological input data:
\begin{itemize}
...
Because the available reanalysis products for surface ozone have their limitations, we deem it necessary to reconstruct the missing data in 2018 for the purpose of DO3SE modeling by applying a Reynold's decomposition of the form:
\begin{equation}
  \chem{[O_3]} = \chem{\left<[O_3]\right>} + \chem{\Delta[O_3]},
  \label{eq:reynolds_decomp}
\end{equation}
with climatology \chem{\left<[O_3]\right>} and annomalies \chem{\Delta[O_3]}.
The exact procedure will be described in the following.
We derive hourly climatologies at all operational stations for July and a combined northern Fennoscandic climatology based on both data from Esrange and Pallas (Appendix Fig.~\ref{fig:ozone_reconstruction_2018_07}a). We compute temporal correlations between Svanvik and the other stations in northern Fennoscandia to estimate a time lag (Appendix Fig.~\ref{fig:time_lag_correlation}. We find $3\,\unit{h}$ for Esrange and Pallas with respect to Svanvik. Due to the high correlation with Svanvik (Appendix~Fig.~\ref{fig:density_distribution}), we decide on Pallas as reference station for the reconstruction. We correct the Fennoscand climatology for the found time lag \chem{\left<[O_3]\right>_{hourly,\,t-3}} by shifting the respective time series back in time by $3\,\unit{h}$ and use it to derive a projection for Svanvik:
\begin{equation}
...
A comprehensive list of model parameters used in this study is found in Supplement~Table~S1. Start ($A_\text{start}$) and end ($A_\text{end}$) of growing season for each species (Norway spruce, birch, perennial grassland) are estimated as follows. Evergreen needleleaf trees are active already at low temperatures \citep{TB:Kolari2007}. This leads to an increase in observable net photosynthesis. We use a MODIS (Aqua/Terra) retreived net photosynthesis product for a $1\times 1\,\unit{km}$ patch around Svanvik and calculate the roots from a quadratic fit through the data (Fig.~\ref{fig:modis_Psn}). We derive $A_\text{start}$ day of the year 122 for 2018 and 106 in 2019 and $A_\text{end}$ 261 and 274, respectively. This means the growing season in 2019 had been one month longer then in 2019. For birch tree $A_\text{start}$, we use the usual 5 consequative days above 5 degrees agricultural rule of thumb based on grided temperature data from seNorge.no. We find 129 and 130, respectively. For perennial grassland, we assume a dormancy of about $1\,\unit{month}$ after snowmelt. Data for first snow-free day is taken from the weather station at Øvre Neiden in Sør-Varanger. Grassland lags about $1\,\unit{month}$ behind birches in both years. As leafs of northern Fennoscadic birches are smaller than southern speciemen, we have measured their dimension for the purpose of this study. %\href{https://www.nina.no/archive/nina/PppBasePdf/oppdragsmelding/829.pdf​}{NINA report} (p. 52)
indicates an average tree height of $13.5\,\unit{m}$ in the area. To derive the leaf area index (LAI), we estimated land use for the $1\times 1\,\unit{km}$ patch around Svanhovd from areal pictures at kartverket.no: $48\,\unit{\%}$ grassland, $32.5\,\unit{\%}$ deciduous, $12.5\,\unit{\%}$ coniferous, $2\,\unit{\%}$ urban. With these, we rescale the average maximum LAI from MODIS ($2.7$). We assumed LAI ratios between the species: $f_\text{con}=5/4\cdot f_\text{dec}$, $f_\text{grass}=7/8\cdot f_\text{dec}$.
Another important factor is the soil texture. As Svanhovd is located in the bed of the Pasvik river, we assume a sandy loam texture.

\subsection{POD and ozone impact}

{\bf TODO: Model results for natural and semi-natural vegetation (birch, pine, grassland)}

\begin{figure*}[t]
  \includegraphics[width=12cm]{DO3SE_results_pody_gsto_o3_Birch}
  \caption{.}
  \label{fig:pody_birch}
\end{figure*}

\begin{figure*}[t]
  \includegraphics[width=12cm]{DO3SE_results_pody_gsto_o3_Norway_spruce}
  \caption{.}
  \label{fig:pody_norway_spruce}
\end{figure*}

\begin{figure*}[t]
  \includegraphics[width=12cm]{DO3SE_results_pody_gsto_o3_Perennial_grass}
  \caption{.}
  \label{fig:pody_grass}
\end{figure*}
