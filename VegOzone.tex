...
To measure the risk of damage inflicted to vegetation through ozone, many different metrics have been defined. Two main types can be differentiated, concentration-based or flux-based metrics, e.g., Accumulated Over a Threshold $\mathrm{x}$ ($\mathrm{AOT_x}$) and Phyto-toxic Ozone Dose over a threshold $\mathrm{y}$ ($\mathrm{POD_y}$), respectively. Due to their simplicity, concentration-based metrics are still widely used, despite the actual integrated flux of ozone entering through the stomata being more relevant for an estimation of the phyto-toxic ozone dose \citep{GCB:Mills2010,ICP:MappingManual2017}.
...
In the following, we derive bespoke parameterizations for the main vegetation types Norway spruce, birch, and perennial grassland. A comprehensive list of model parameters used as starting point in this study can be found in Supplement~Table~S1. Default PFTs are derived from \citet{ICP:MappingManual2017} and will be referred to as \emph{mapping manual}.

These suggested, hypothetical PFTs or species parameterizations ought to be better adapted to the climatic conditions in the target region, but have to be varified by experiements. We hypothize that plants are likely to optimize their growth throughout the short growing season and hence the uptake of \chem{CO_2}. For simplicity, we assume a first order proptionality between photosynthesis and $g_\mathrm{sto}$. Plants are therefor likely to optimize their stomatal conductance with respect to climatological drivers.
In Fig.~\ref{fig:f_temp_grassland}, we display the construction of bespoke PFTs or species temperature and light response function $f_\mathrm{T}$ and $f_\mathrm{light}$ for grassland (for Norway spruce and birch see Appendix~\ref{fig:f_temp_spruce_birch}). The response functions are shown together with climatology probability density functions (PDFs) of temperature and global radiation, respectively. In both cases, a parameterization is considered more optimized the larger the intersection area between the response function and PDF is. With respect to temperature (Fig.~\ref{fig:f_temp_grassland}a)), we assume two different types: \emph{boreal} and \emph{cold}. In both cases, we assume that the species are more tolerant to cold conditions than in the default mapping manual parameterization and shift $f_\mathrm{T}$ towards colder temperature. Herein, \emph{cold} represents a species that is more tolerant to warmer temperatures, but slightly less tolerant to cold temperatures than \emph{boreal}. For Norway spruce, $T_\mathrm{opt}$ are estimated from a longer timeseries of observed \chem{CO_2} uptake over temperature at two observations sites in Finland presented in \citet{TB:Kolari2007}. For the the senisitivity regarding light, we assume that the plants may react either faster or slower to lower light intensities. We qualitatively quantify this by a $\pm 20\,\%$ shift in respective halfway PPFD($f_\mathrm{light}=0.5$). The resulting functions are shown in Fig.~\ref{fig:f_temp_grassland}b). All derived parameters are given in Tables~\ref{tab:sensitivity_tests_temp}--\ref{tab:sensitivity_tests_light}.
