...
\section{Results}
\label{sec:results}
In this section, we first estimate the multiannual average stomatal conductance $\left<g_\mathrm{sto}\right>$ relative to $g_\mathrm{max}$ for each PFT from the long term meteorological data according to Eq.~(\ref{eq:stomatal}) and compare the different local climate derived parameterizations to MM (Section~\ref{subsec:res_local}). We also use the $\mathrm{DO_3SE}$ model to compute the ozone dose and predicted biomass loss due to ozone for the two recent GSs 2018 and 2019 for each species and set of local parameters (Section~\ref{subsec:do3se_results}). Finally, we put the results on ozone damage risk in 2018 and 2019 into a climatological perspective meaning how common these conditions have been in the past. To this end, we analyze both the past seasonality and the deviation from the norm of temperature, precipitation radiant energy, and ozone for these two years (Section~\ref{subsec:climatologies}).
